const {SignIn,SignUp,update_posts,update_profile,add_posts,get_posts,get_restaurents,delete_profile,remove_posts,deleteOnePost,addComments} =require("../../modules/restaurents/index")
...
const signup=(req,res)=>{
  ...
  const add_post = (req, res) => {
    ...
    add_posts((err, result) => {
      console.log(result, "hello");
      err ? res.status(404).send(err) : res.status(200).send(result);
    }, req.body, restaurantId); // pass restaurant ID to add_posts function
  };
...
const deleteOnePoste=(req,res)=>{
    console.log('hi')
   deleteOnePost((err,results)=>{
        err ?  res.status(500).send(err) : res.status(201).send(results)
    },req.params.idRes,req.params.idPost)
}

const update_post=(req,res)=>{
    console.log(req.params);
    update_posts((err,result)=>{
        err ? res.status(404).send(err) : res.status(200).send(result)
    },req.body,req.params.id,req.params.idRes)
}
